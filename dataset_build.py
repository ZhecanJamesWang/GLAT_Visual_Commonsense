...
def preprocess_data(new_categories, rel_data):
    ...
    for idx in tqdm(range(len(rel_data[:50]))):
...
if not os.path.exists(save_root):
    os.mkdir(save_root)
filename = os.path.join(save_root, 'train_VG_clean_50.pkl')
with open(filename, 'wb') as f:
    pickle.dump(train_data, f)

filename = os.path.join(save_root, 'test_VG_clean_50.pkl')
with open(filename, 'wb') as f:
    pickle.dump(test_data, f)

filename = os.path.join(save_root, 'vocab_clean_50.pkl')
with open(filename, 'wb') as f:
