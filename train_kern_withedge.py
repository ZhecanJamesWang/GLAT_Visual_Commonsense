...
# from pygcn.utils_kern import load_data, accuracy
...
def get_edge_info(pad_masks, adjs_connect, adjs_label):
    ...
    adjs_connect = adjs_connect.data.cuda()
    ...
    if len(torch.nonzero(neg_mask)) > len(torch.nonzero(pos_mask)):
        ...
        bal_neg_mask[torch.nonzero(neg_mask).squeeze(-1)[mask]] = 0
...
def train(epoch):
    ...
    loss_totoal_con = 0
    num_sample = 0
    node_acc_entity = utils_kern.Counter(classes=vocab_num[1])
    node_acc_predicate = utils_kern.Counter(classes=vocab_num[0])
    node_acc_mask_entity = utils_kern.Counter(classes=vocab_num[1])
    node_acc_mask_predicate = utils_kern.Counter(classes=vocab_num[0])
    node_acc_mask_all = utils_kern.Counter(classes=vocab_num[1])
    edge_acc_train = utils_kern.Counter(classes=3)
    edge_acc_eff = utils_kern.Counter(classes=3)
    ...
    for i, (gt_class, input_class, adj_con, adj_lbl, input_mask, node_type) in enumerate(train_loader):
        ...
        if adj_con.size(1) == 1:
            ...
            # pdb.set_trace()
            pred_label_mask_predicate = pred_label_flat[((input_mask.view(-1) == 1).data.long() * (node_type.view(-1) == 0).long()).byte()]
            gt_class_mask_predicate = gt_class_flat[((input_mask.view(-1) == 1).data.long() * (node_type.view(-1) == 0).long()).byte()]

            pred_label_mask_entity= pred_label_flat[((input_mask.view(-1) == 1).data.long() * (node_type.view(-1) == 1).long()).byte()]
            gt_class_mask_entity = gt_class_flat[((input_mask.view(-1) == 1).data.long() * (node_type.view(-1) == 1).long()).byte()]

            pred_label_mask_all = pred_label_flat[((input_mask.view(-1) == 1).data.long()).byte()]
            gt_class_mask_all = gt_class_flat[((input_mask.view(-1) == 1).data.long()).byte()]
            assert pred_label_mask_all.size(0) == pred_label_mask_entity.size(0) + pred_label_mask_predicate.size(0)
            ...
            if (i+1) % 100 == 0:
                print('Train Epoch: {:04d} [{}/{}] '.format(epoch, i, len(train_loader)),
                      ...
                      'node_acc_mask_all_train: {:.4f} '.format(node_acc_mask_all.overall_acc()),
    ...
    writer.add_scalar('train/node_acc_mask_train_alls', node_acc_mask_all.overall_acc(), epoch)
...
def test(epoch):
    ...
    # node_acc = utils_kern.Counter(classes=vocab_num[0])
    # node_acc_mask = utils_kern.Counter(classes=vocab_num[0])
    edge_acc = utils_kern.Counter(classes=3)
    ...
    print('Test Epoch Finished: {:04d} '.format(epoch),
      'loss_rec: {:.4f} '.format(loss_total_rec/num_sample),
      'loss_con: {:.4f} '.format(loss_totoal_con/num_sample),
          'node_acc_entity_test: {:.4f} '.format(node_acc_entity.overall_acc()),
          'node_acc_predicate_test: {:.4f} '.format(node_acc_predicate.overall_acc()),
          'node_acc_mask_entity_test: {:.4f} '.format(node_acc_mask_entity.overall_acc()),
          'node_acc_mask_predicate_test: {:.4f} '.format(node_acc_mask_predicate.overall_acc()),
    ...
    save_to_record("".join(['Test Epoch Finished: {:04d} '.format(epoch),
      ...
      'node_acc_mask_predicate_test: {:.4f} '.format(node_acc_mask_predicate.overall_acc()),
      'node_acc_mask_entity_test: {:.4f} '.format(node_acc_mask_entity.overall_acc()),
      'node_acc_mask_all_test: {:.4f} '.format(node_acc_mask_all.overall_acc()),
      'edge_acc_eff_overallacc: {:.4f} '.format(edge_acc.overall_acc()),
      'edge_acc_eff_classacc: {:.4f} {:.4f} {:.4f} '.format(edge_acc.class_acc()[0],
                                                            edge_acc.class_acc()[1],
                                                            edge_acc.class_acc()[2]),
    ...
    writer.add_scalar('train/node_acc_test_entity', node_acc_entity.overall_acc(), epoch)
    writer.add_scalar('train/node_acc_test_predicate', node_acc_predicate.overall_acc(), epoch)
    writer.add_scalar('train/node_acc_mask_test_entity', node_acc_mask_entity.overall_acc(), epoch)
    writer.add_scalar('train/node_acc_mask_test_predicate', node_acc_mask_predicate.overall_acc(), epoch)
    writer.add_scalar('train/node_acc_mask_test_all', node_acc_mask_all.overall_acc(), epoch)
    ...
    if acc_recorder_pred.compare_node_mask_acc(node_acc_mask_predicate.overall_acc()):
        utils_kern.save_model(model, epoch, "best_test_node_mask_predicate_acc", args.model_outdir, record_file_name.split(".")[0],
                        acc_recorder_pred.get_best_test_node_mask_acc())

    if acc_recorder_entity.compare_node_mask_acc(node_acc_mask_entity.overall_acc()):
        utils_kern.save_model(model, epoch, "best_test_node_mask_entity_acc", args.model_outdir, record_file_name.split(".")[0],
                        acc_recorder_entity.get_best_test_node_mask_acc())

    if acc_recorder_all.compare_node_mask_acc(node_acc_mask_all.overall_acc()):
